...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    means.map(_ -> GenSeq()).toMap ++ points.groupBy(p => findClosest(p, means))
  }
  ...
  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    oldMeans.zip(newMeans).forall(lessThanOrEqual(eta))
  }

  def lessThanOrEqual(eta: Double)(points: (Point, Point)): Boolean = {
    points._1.squareDistance(points._2) <= eta
  }
